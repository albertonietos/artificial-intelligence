...
on markov decision processes implemented using the class mdp.MDP (see mdp.py)

The function `value_iteration` performs the value iteration steps on a 
decision process until the sensitivity condition given by epsilon is met
...
def value_of(mdp, s, a, v, gamma):
    ...
    for s2 in mdp.successor_states(s, a):
        res += mdp.P(s,a,s2) * (mdp.R(s,a,s2) + gamma * v[s2])
    return res
        
def value_iteration(mdp, gamma, epsilon = 0.001):
    """
    Perform value iteration of Markov Decision Process MDP.
    ...
    # CODE HERE CODE HERE CODE HERE
    #
    # Specify tolerance and residual
    tol = epsilon * (1 - gamma) / (2 * gamma)
    residual = dict()

    # Initialize (with 0s) dictionaries with current and next value
    v = dict.fromkeys(mdp.states(), 0)
    v_next = dict.fromkeys(mdp.states(), 0)

    # Step 2 - Find the best possible action for every state
    while True:
        for s in mdp.states():
            v_temp_lst = []
            for a in mdp.applicable_actions(s):
                v_temp = value_of(mdp, s, a, v, gamma)
                v_temp_lst.append(v_temp)
            # Select action with maximum value
            v_next[s] = max(v_temp_lst)
            residual[s] = abs(v_next[s] - v[s])

        # Copy values for next iteration
        v = v_next.copy()

        # Step 3 & 4 - Check tolerance and end if tolerance is met
        if residual[argmax(residual)] < tol:
            break # v is the chosen policy
    return v
