...
def get_conditional_probability(data, query, conditions):
    ...
    return get_data_frequency(data, combined_dictionary) / (get_data_frequency(data, conditions) + epsilon)
    # =========================================================
...
def exhaustive_enum(G, X, E, e):
    ...
    for assignment in assignments:
        ...
        for node in G.nodes:
            ...
            # =========================================================
            # CODE HERE!
            # Accumulate product of each node
            tmp *= evaluate_node(G, query=node.data_id, val=combined_dictionary[node.data_id], conditions=combined_dictionary)
            # =========================================================
        # Accumulate sum
        total += tmp
    return total
...
def sample(G, X, E, e):
    ...
    for variable in X:
        ...
        if variable in E:
            ...
            # =========================================================
            # CODE HERE
            w *= evaluate_node(G, query=variable, val=value, conditions=parents)
            # =========================================================
        else:
            # =========================================================
            # CODE HERE
            prob = G.get_node_probability(feature=variable, conditions=parents)
